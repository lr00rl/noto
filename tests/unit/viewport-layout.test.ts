...
describe('which top level blocks stay fully painted', () => {
  it('keeps the selection and its neighbours', () => {
    ...
    expect(liveTopLevelIndices(state.doc, state.selection)).toEqual([0, 1, 2, 3, 4]);
    expect(liveClasses(state)).toEqual([
      LAYOUT_LIVE_CLASS, LAYOUT_LIVE_CLASS, LAYOUT_LIVE_CLASS, LAYOUT_LIVE_CLASS, LAYOUT_LIVE_CLASS,
    ]);
  });

  it('does not walk past the ends of the document', () => {
    const atStart = stateFor(FIVE, 0);
    expect(liveTopLevelIndices(atStart.doc, atStart.selection)).toEqual([0, 1, 2]);
    const atEnd = stateFor(FIVE, 4);
    expect(liveTopLevelIndices(atEnd.doc, atEnd.selection)).toEqual([2, 3, 4]);
  });
  ...
  it('moves the live window when the caret does', () => {
    ...
    expect(liveTopLevelIndices(state.doc, state.selection)).toEqual([0, 1, 2]);
    let position = 0;
    for (let index = 0; index < 3; index += 1) position += state.doc.child(index).nodeSize;
    state = state.apply(state.tr.setSelection(TextSelection.create(state.doc, position + 1)));
    expect(liveTopLevelIndices(state.doc, state.selection)).toEqual([1, 2, 3, 4]);
  });
  ...
  it('stays cheap on a long document: only the window is decorated', () => {
    ...
    expect(liveTopLevelIndices(state.doc, state.selection)).toEqual([248, 249, 250, 251, 252]);
    expect(liveClasses(state)).toHaveLength(5);
  });
});
